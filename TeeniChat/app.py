...
def chat():
    ...
    avatar_url = (
        url_for("uploaded_file", filename=user["avatar"])
        if user and user["avatar"]
        else url_for("static", filename="default-avatar.png")
    )
...
def api_messages():
    ...
    if request.method == "POST":
        ...
        with get_db() as conn:
            conn.execute(
                "INSERT INTO messages (sender, content, file_path, voice_path) VALUES (?,?,?,?)",
                (sender, content, file_path, voice_path),
            )
            conn.commit()
        ...
        with get_db() as conn:
            msgs = conn.execute(
                "SELECT * FROM messages ORDER BY id ASC LIMIT 200"
            ).fetchall()
        out = []
        for m in msgs:
            out.append(dict(m))
